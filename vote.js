...
module.exports = function(io, socket, context) {
  socket.on('voteEvent', async (data) => {
    ...
    // 투표(선택/취소)
      ...
      if (idx >= 0) {
        const vote = votes[idx];
        const userIdx = vote.users.findIndex(u => u.uId === context.getCurrentUserId());
        // 기존 투표 내역 있으면 취소
        if (userIdx >= 0) {
          ...
          await context.queryPromise(
            ...
            [node, currentProjectId, currentTeamId, context.getCurrentUserId()]
          );
        }
        // 새 항목 선택
        if (num >= 1 && num <= 4) {
          vote.count[num - 1] += 1;
          vote.users.push({ uId: context.getCurrentUserId(), num });
          await context.queryPromise(
            'INSERT INTO VoteUser (node, pId, tId, uId, num) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE num = VALUES(num)',
            [node, currentProjectId, currentTeamId, context.getCurrentUserId(), num]
          );
        }
        // count 업데이트
        await context.queryPromise(
